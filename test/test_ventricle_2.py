...
def main():
    ...
    #Obtain the indexes of the non-void nodes
    v_indexes = tissue.GetNodeIndex()
    ...
    while tissue.GetTime() < 1000.0:
        ...
        if tick == arritmic3d.SystemEventType.EXT_ACTIVATION:
            ...
            np.put(grid.point_data['State'], v_indexes, tissue.GetStatesIndexed())
            np.put(grid.point_data['APD'], v_indexes, tissue.GetAPDIndexed())
            np.put(grid.point_data['CV'], v_indexes, tissue.GetCVIndexed())
