# Arritmic3D
A fast Eikonal computational model for electrophysiology simulation. Version for atrial arrhythmias.
...
* `restitutionSurfaces` folderInside this folder are files in the format `RestitutionSurface_A_B_C.csv` where A={BZ, Sanas} (respectively: Border Zone, Helthy); B={APD, CV} and C={Endo, Epi, Mid}
...
Once all the data has been copied to the directory `arritmic3D`, open `arritmic3D/paramsInit.dat` with a text editor. There, the case to be simulated is configured. Se the `case` parameter to 0 in case you want to simulate an atria case, and set it to 1 if you want a tissue slab to be simulated. The parameter `folderCase` indictes the name of the subdirectory inside `arritmi3D/casos/` that will be simulated.

````
case           :  0                         ### Case options: ATRIA -> 0 / BLOCK_VTK -> 1
folderCase     :  Atlas_Krueger2013    ### FolderCase: The case folder name located in arritmic3D/casos
````

Next, open `params.dat` file located in `arritmic3D/casos/<folderCase>` with an editor, and modify the case parameters as you need. The definition of each parameter is detailed below in the comments:

````
model                   :   Bloque.stl                  #  Polydata mesh name to display
hay_mesh                :   true                        #  If there is Polydata mesh to display
restsurf 		: true 			  #  Restitution Surface APD property -> true ; Restitution Curve APD property -> false
...
multi_view              :   false                       #  Shows front and back view at the same time
...
di_Sana_Init            :   360                         #  Diastolic interval of healthy cells to define initial state
di_BZ_Init              :   185                         #  Diastolic interval of border zone cells to define initial state
apd0_Sana_Init            :   106                         #  Previous APD of healthy cells to define initial state
apd0_BZ_Init              :   269                         #  Previous APD of border zone cells to define initial state
stimFrecS1              :   500                         #  Stimulus frequency for S1
stimFrecS2              :   300                         #  Stimulus frequency for S2
nStimsS1                :   3                           #  Number of S1 stimuli (The cellular automaton only needs 2 S1 stimuli to stabilize)
nStimsS2                :   2                           #  Number of S2 stimuli
apd_memory              :   0.0                         #  % APD memory applied to new activation (supported values from 0-1, 0%-100%)
cv_memory               :   0.05                        #  % Conduction velocity memory applied to new activation (supported values from 0-1, 0%-100%)
reductCVtr              :   0.70                        #  % Reduction of the conduction velocity in the transverse direction to the fiber orientation (supported values from 0-1, 0%-100%) Ex: 0.70, applies in the transverse direction 70% of the longitudinal conduction velocity. This is used when all the nodes are required to have the same reduction value.
...
active_BloqueVent_extraI  :   false	#  Do not modify, currently disabled
fileName_BloqueVent_extraI  :   nodes_CasoBloqueVent.txt  	  # Define the name of the file containg the nodes of the block activated to generate re-entrance
````
...
* The application has different functionalities that are activated by pressing some keys while the 'arritmic3D' display window is active:
    ...
    * 'g' key: Record sequence of simulation images to build video start/stop ('f' key must be on). Files saved in arritmic3D/video/videoFrames**
    * 'c' key: Generate Ensight files to save Life time animation start/stop. Saved in folder arritmic3D/casos/<folderCase>/Case_***
    ...
    * 'x' key: Generate Ensight files to save Activation maps on each new stimulus start/stop. Saved in folder arritmic3D/casos/<folderCase>/A_Map_***
...
* If there is an error after pressing the Run button to start the simulation, it can be related to the frame rate definition. You can try by changing the line
	...
	<code>frameRate(1000);</code> (or whatever the maximum frame rate you want it to be)

	in the last line of the <code>setup()</code> function.

* If the programme crashes after pressing Run, close and reopen the programme
* If the program throws an error stating that it cannot find a library, check the installation steps to see if all the libraries have been installed.
